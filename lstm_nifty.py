...
df = pd.read_csv('/kaggle/input/nifty-50-minute-data/NIFTY 50_minute_data.csv')
...
def visualize_predictions(model, test_loader, device='cpu'):
    ...
    print(f"Coefficient of Variation: {rmse / np.mean(actuals):.2f}")
    ...
    plt.ylabel("Price")
    plt.legend()
    plt.title("Nifty 50 Intraday Price Prediction")
    plt.show()

visualize_predictions(model, test_loader, device=device)

torch.save(model.state_dict(), "lstm_nifty.pth")